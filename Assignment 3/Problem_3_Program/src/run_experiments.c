...
int main(int argc, char **argv)
{
	...
	while((dptr=readdir(dir_p))!=NULL)
	{
		...
		while(fgets(line, sizeof(char) * params[0] * 10, data_file))
		{
			...
			dc(arr, 0, params[0]-1, &max_sum, &max_li, &max_ri);
			...
			gettimeofday(&start, NULL);
			dp(arr, params[0], &max_sum, &max_li, &max_ri);
			gettimeofday(&end, NULL);
			fprintf(output_file_dp, "%.2f,%d,%d,%ld\n", max_sum, max_li+1, max_ri+1, (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
			// printf("%.2f,%d,%d,%ld\n", max_sum, max_li, max_ri, (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
